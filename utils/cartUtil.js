 ...
 * 请求购物车数据
 */
export function getCartProducts(resultCallback) {
  util.request(`${api.CartProductInfo}`).then(res => {
    ...
    if (Array.isArray(cart_products) && cart_products.length > 0) {
      ...
      resultCallback(resultCartProducts);
    }
    //请求购物车产品详情
    if (Array.isArray(cart_products) && cart_products.length > 0) {
      cart_products.forEach(cart_product => {
        const { id, product_id, color_id, size_id } = cart_product;
        let product_info_sucess = false, color_info_success = false, size_info_success = false, count_info_success = false;
        //请求product_id对应产品信息
        util.request(`${api.GoodDetail}?id=${product_id}`, {'expand': 'picUrl'}).then(res => {
          ...
          updateCartProductInResultCartProducts(result_cart_product_info, resultCartProducts);
 ...
 * 更新购物车详情
 */
function updateCartProductInResultCartProducts(cart_product_info, resultCartProducts) {
...
export function removeCartList(id) {
  ...
  util.request(`${api.DeleteCartProduct}?id=${id}`, undefined, 'DELETE').then(res => {
